...
int main(int argc, char **argv)
{
   ...
   for (int id=1;id<size;id++){
      printf("yello");
      end_array[id] = end;
     // MPI_Send(&my_world[0][end],(sz_y/size+2)*sizeof(my_world[0][0]),MPI_INT,id,0,MPI_COMM_WORLD);
   //   MPI_Send(&my_world[1][end],(sz_y+2/size)*(sz_x+2),MPI_INT,id,0,MPI_COMM_WORLD); 
      end= end + sz_y/size+2;  
     }
}
   printf("hiiii"); 
   int sub_size_with_boarders = (sz_x+2)*(sz_y+2)/size;
   char * sub_world_mem = (char *) malloc(2*sub_size_with_boarders*sizeof(char));
   for (int i=0; i<sub_size_with_boarders; i++)
   	sub_world_mem[i] = 0; 

   char ** sub_world[2];
   sub_world[0] = (char **) malloc((sz_y+2)/size * sizeof(char*));
   sub_world[1] = (char **) malloc((sz_y+2)/size * sizeof(char*));
   for (int which=0; which < 2; which++) {
      for (int r=0; r<(sz_y)/size+2; r++)
         sub_world[which][r] = &sub_world_mem[(r*(sz_x+2))+which*sub_size_with_boarders];
   }
   printf("After Allocate\n");
 
  // MPI_Recv(&sub_world,(sz_y+2/size)*sizeof(my_world[0][0]),MPI_INT,0,0,MPI_COMM_WORLD, &status);
 //Main Time loop
   for(int t=0; t<num_loops;t++) {
        ...
        for (int c=1; c<sz_x+1; c++){
           sub_world[which][0][c] = my_world[which][sz_y][c];
           sub_world[which][sz_y+1][c] = my_world[which][1][c];
        }
   	for (int r=1; r<sz_y+1; r++){
           sub_world[which][r][0] = my_world[which][r][sz_x];
           sub_world[which][r][sz_x+1] = my_world[which][r][1];
        }
